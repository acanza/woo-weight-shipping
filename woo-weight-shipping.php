...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	...
	function woo_weight_shipping_init(){
		...
		if( !class_exists( 'WC_Woo_Weight_Shipping' ) ){

			class WC_Woo_Weight_Shipping extends WC_Shipping_Method{
				...
				*/
				public function __construct() {
					...
					add_filter( 'woocommerce_package_rates', array( $this, 'set_shipping_method_availability' ), 10, 2 );
				}
				...
				*/
				function set_shipping_method_availability( $rates, $package ){

					if( $package[ 'destination' ][ 'country' ] != $this->countries ){

						unset( $rates[ 'increase_rate' ] );
					}
				...
				function process_special_regions_rates() {
					...
					if( isset( $_POST[ $this->id . '_regions'] ) ){
						...
						foreach ( $regions_list as $region => $data ){
							...
							if ( isset( $data['weight'] ) & isset( $data['cost'] ) ){

								$max = count( $data['weight'] );
								$cont = 0;
								for($i=0; $i<=$max; $i++){
									
									if ( isset( $data['cost'][$i] ) || isset( $data['weight'][$i] ) ) {
	...
	add_action( 'woocommerce_shipping_init', 'woo_weight_shipping_init' );
